...
<div class="col-md-12">
<form name="frm" action="{{ route('goLinkExamFinish') }}" method="post" id="quesFrm">
{{ csrf_field() }}
	<div class="row">
		<div class="col-md-3">
			...
			<input type="hidden" name="exam_link_id" value="{{ $exam_link_id }}">
		</div>
		<div class="col-md-6">
...
function isLiveCandidate() {
	$.ajax({
		type: "POST",
		url: "{{ route('isLiveCandidate') }}",
		data: {
			"_token" : "{{ csrf_token() }}",
			"user_id" : "{{ $USER_ID }}" 
		},
		success: function(rtn) {
			if (rtn == 1) {
				alert('Exam has Stop By Administrator. Please Contact To Administrator');
